...
const AttendanceView = {
    ...
    container.innerHTML = `
      <div class="space-y-6 animate-fade-in max-w-4xl mx-auto">
        ...
        <div id="att-sheet-container" class="space-y-6">
          <div class="p-12 text-center text-slate-400 text-sm bg-white rounded-3xl border border-slate-100 shadow-sm">
      ...
      select.addEventListener('change', (e) => {
        ...
        if (sid) {
          ...
          container.querySelector('#att-sheet-container').innerHTML = `
            <div class="p-12 text-center text-slate-400 text-xs bg-white rounded-3xl border border-slate-100 shadow-sm">
              ${isAr ? 'يرجى اختيار حصة من القائمة أعلاه.' : 'Veuillez sélectionner une séance ci-dessus.'}
            </div>
          `;
        }
      });
  ...
  async loadSessionSheet(container, sessionId) {
    ...
    try {
      sheet.innerHTML = `<div class="p-12 text-center text-slate-400 text-sm animate-pulse">${isAr ? 'جاري تحميل ورقة الحضور...' : 'Chargement de la feuille de présence...'}</div>`;
      ...
      if (!data.students || data.students.length === 0) {
        sheet.innerHTML = `
          <div class="p-12 text-center bg-white rounded-3xl border border-slate-200">
            ...
            <p class="text-sm font-bold text-slate-700">${isAr ? `لا يوجد تلاميذ مسجلين في الفوج "${data.group_name}".` : `Aucun élève inscrit dans le groupe "${data.group_name}".`}</p>
            <p class="text-xs text-slate-400 mt-1">${isAr ? 'يرجى تسجيل تلاميذ في هذا الفوج من صفحة التلاميذ.' : 'Affectez d\'abord des élèves à ce groupe depuis la page Élèves.'}</p>
          </div>
        `;
        if (window.lucide) lucide.createIcons();
        return;
      ...
      sheet.innerHTML = `
        ...
        <div class="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-700">${levelLabel}</span>
                ${data.is_recorded ? `<span class="text-xs font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">✅ ${isAr ? 'تم التسجيل مسبقاً' : 'Déjà pointé'}</span>` : ''}
              </div>
              <p class="text-xs text-slate-500 mt-0.5">📅 ${data.date} • 🕒 ${data.start_time} - ${data.end_time} • ${data.students.length} ${I18n.t('students')}</p>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            <div>
              <label class="block text-xs font-bold text-slate-700 uppercase mb-1">${I18n.t('topic')}</label>
              <input id="att-topic-input" type="text" value="${data.topic || ''}" placeholder="${isAr ? 'مثال: الأعداد المركبة، المتتاليات، الهندسة...' : 'ex: Nombres complexes, suites numériques, géométrie...'}" class="w-full px-3.5 py-2 text-xs sm:text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 bg-slate-50/50">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-700 uppercase mb-1">${I18n.t('notes')}</label>
              <input id="att-notes-input" type="text" value="${data.notes || ''}" placeholder="${isAr ? 'مثال: تم إكمال السلسلة رقم 3...' : 'ex: Série d\'exercices n°3 terminée...'}" class="w-full px-3.5 py-2 text-xs sm:text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 bg-slate-50/50">
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-3">
          <div class="flex items-center justify-between pb-3 border-b border-slate-100 text-xs font-bold text-slate-500">
            <span>${I18n.t('students')}</span>
            <div id="att-summary-counts" class="flex items-center gap-2 sm:gap-3 flex-wrap">
              <!-- Real-time counters -->
            </div>
          </div>
              ...
              <div class="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 student-att-row" data-student-id="${st.student_id}">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-xl bg-slate-100 text-slate-700 font-bold flex items-center justify-center text-xs">
                    ${st.student_name ? st.student_name.charAt(0) : 'E'}
                  </div>
                  <div>
          ...
          <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
            <span class="text-xs text-slate-400">${isAr ? 'المجموع' : 'Total'} : <strong>${data.students.length} ${I18n.t('students')}</strong></span>
